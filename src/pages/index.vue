                ...
                <div class="position-grid">
                  <div v-for="i in 12" :key="i" class="position-slot"
                    ...
                    <div v-if="takimB.oyuncular.CM[i-1]" 
                         ...
                         :data-seviye="takimB.oyuncular.CM[i-1].pozisyonlar?.CM || 1"
                         draggable="true"
                ...
                <div class="position-grid">
                  <div v-for="i in 4" :key="i" class="position-slot"
                    ...
                    <div v-if="takimB.oyuncular.DC[i-1]" 
                         ...
                         :data-seviye="takimB.oyuncular.DC[i-1].pozisyonlar?.DC || 1"
                         draggable="true"
...
// Mevki katsayıları için tip tanımı
interface MevkiKatsayi {
  mevki: string
  yetenek: string
  katsayi: number
...
// Pozisyon seviyesi katsayıları
const pozisyonKatsayilari = {
  5: 1,    // Çok İyi
  4: 0.9,  // İyi
  3: 0.7,  // Orta
  2: 0.5,  // Zayıf
...
const oyunculariYukle = async () => {
  try {
    ...
    havuzdakiOyuncular.value = oyuncular.map(o => {
      // API'den gelen yetenekleri doğru formata dönüştür
      const yetenekler: { [key: string]: number } = {}
      if (Array.isArray(o.yetenekler)) {
        o.yetenekler.forEach((yetenek: any) => {
          if (yetenek.isim && typeof yetenek.seviye === 'number') {
            yetenekler[yetenek.isim] = yetenek.seviye
          }
        })
      }
...
// Oyuncu gücünü hesapla
const hesaplaOyuncuGucu = async (oyuncu: Oyuncu, pozisyon: keyof Pozisyonlar) => {
  try {
    // Mevki katsayılarını API'den al
    const mevkiKatsayilari = await api.getMevkiKatsayilari(pozisyon)
    
    if (!mevkiKatsayilari || !oyuncu.yetenekler) return oyuncu.guc || 0

    // Toplam puanı hesapla
    let toplamPuan = 0
    const toplamKatsayi = mevkiKatsayilari[0]?.toplam_katsayi || 1

    // Her yetenek için puan hesapla
    mevkiKatsayilari.forEach((mk: MevkiKatsayi) => {
      const yetenekDegeri = oyuncu.yetenekler?.[mk.yetenek] || 0
      toplamPuan += yetenekDegeri * mk.katsayi
    })

    // Formülü uygula: (Toplam Puan / Toplam Katsayı) * 5
    let guc = (toplamPuan / toplamKatsayi) * 5

    // Pozisyon seviyesine göre çarp
    const pozisyonSeviyesi = (oyuncu.pozisyonlar?.[pozisyon] || 1) as keyof typeof pozisyonKatsayilari
    guc *= pozisyonKatsayilari[pozisyonSeviyesi]

    // Sonucu yuvarla
    return Math.round(guc)
  } catch (error) {
    console.error('Güç hesaplanırken hata:', error)
    return oyuncu.guc || 0
  }
}

// dropOnArea fonksiyonunu güncelle
const dropOnArea = async (event: DragEvent, takimKodu: 'A' | 'B', pozisyon: keyof Pozisyonlar, hedefIndex: number) => {
  ...
  // Oyuncunun yeni gücünü hesapla
  const yeniGuc = await hesaplaOyuncuGucu(oyuncu, pozisyon)
  oyuncu.guc = yeniGuc
